package tsms.controls;

import java.util.HashMap;
import java.util.Map;

import tsms.annotation.Component;
import tsms.bind.DataBinding;
import tsms.dao.EnglishBoardDao;

@Component("/english/list.do")
public class EnglishBoardListController implements Controller, DataBinding {
	EnglishBoardDao englishBoardDao;

	public EnglishBoardListController setEnglishBoardDao(EnglishBoardDao englishBoardDao) {
		this.englishBoardDao = englishBoardDao;
		return this;
	}

	@Override
	public Object[] getDataBinders() {
		return new Object[] { "searchType", String.class, "search", String.class, "go", String.class, "gogroup",
				String.class };
	}

	@Override
	public String execute(Map<String, Object> model) throws Exception {
		String searchType = (String) model.get("searchType");
		String search = (String) model.get("search");
		String go = (String) model.get("go");
		String gogroup = (String) model.get("gogroup");

		HashMap<String, Object> paramMap = new HashMap<String, Object>();
		if (search != null) {
			paramMap.put("searchType", searchType);
			paramMap.put("search", search);
		}

		paramMap = englishBoardDao.selectList(go, gogroup, paramMap);
		model.put("page", paramMap.get("page"));
		model.put("englishs", paramMap.get("englishs"));

		return "/english/EnglishBoardList.jsp";

	}
}
